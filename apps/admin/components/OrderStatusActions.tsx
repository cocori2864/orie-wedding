        ...
        <div className="space-y-3">
            ...
            {currentStatus === 'payment_pending' && (
                <>
                    ...
                    <button
                        onClick={() => updateStatus('pending', '예약 대기 상태로 되돌렸습니다.')}
                        disabled={loading}
                        className="w-full py-2 border border-gray-300 text-gray-600 rounded-md hover:bg-gray-50 disabled:opacity-50 mt-2 text-sm"
                    >
                        ↩ 이전 단계로 (예약 대기)
                    </button>
                </>
            ...
            {currentStatus === 'confirmed' && (
                <>
                    ...
                    <button
                        onClick={() => updateStatus('payment_pending', '예약금 대기 상태로 되돌렸습니다.')}
                        disabled={loading}
                        className="w-full py-2 border border-gray-300 text-gray-600 rounded-md hover:bg-gray-50 disabled:opacity-50 mt-2 text-sm"
                    >
                        ↩ 이전 단계로 (예약금 대기)
            ...
            {finalPaymentStatus === 'verification_pending' && (
                <>
                    ...
                    <button
                        onClick={() => updateStatus('confirmed', '예약 확정 상태로 되돌렸습니다.')}
                        disabled={loading}
                        className="w-full py-2 border border-gray-300 text-gray-600 rounded-md hover:bg-gray-50 disabled:opacity-50 mt-2 text-sm"
                    >
                        ↩ 이전 단계로 (예약 확정)
            ...
            {currentStatus !== 'cancelled' && (
                <button
                    ...
                    disabled={loading}
                    className="w-full py-2 border border-red-200 text-red-600 rounded-md hover:bg-red-50 disabled:opacity-50 mt-4"
                >
                    주문 취소
                </button>
            )}
